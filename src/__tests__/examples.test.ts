...
describe(`miscellaneous tests, jff`, () => {
  test(`A vat calculator`, () => {
    const p = (vat: number) => pipe(misc.useCallValue<number>()).pipe(math.divideBy(100)).pipe(math.multiplyBy(vat)).toSyncFunction();
  ...
  test(`Pipeline composition`, () => {
    ...
    const p2 = pipe(misc.useCallValue()).pipe(math.divideBy(2)).pipe(math.pow(3));
    const p3 = pipe<number, number>(misc.useCallValue()).pipe(misc.applySync(p1)).pipe(misc.applySync(p2));
    const p4 = pipe<number, number>(misc.useCallValue()).pipe(misc.applySync(p3)).pipe(type.toString()).pipe(string.chars());
