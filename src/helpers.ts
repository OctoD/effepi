...
export function throwContextExecutionFlow(
  ...
  if (context.executionFlow !== flow) {
    throw new Error(
      `Cannot use ${functionName} on ${context.executionFlow} context`
    );
  }
}

export function throwIfNotArray(
  functionName: string,
  value: unknown
): never | void {
...
export function throwIfNotFunction(
  ...
  if (typeof value !== 'function') {
    throw new TypeError(
      `${functionName}'s ${paramName} argument must be a function`
    );
  }
}

export function throwIfNotObject(
  functionName: string,
  value: unknown
): never | void {
